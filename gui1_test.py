...
# Function to generate an AI-generated sentence
def generate_ai_sentence(input_text):
    try:
        response = openai.Completion.create(
            engine="text-davinci-003",
            prompt=f"Form a coherent sentence from the following input: {input_text}",
            max_tokens=50
        )
...
# Function to translate and speak text in Hindi
def translate_and_speak_hindi():
    ...
    try:
        ...
        engine.runAndWait()
        output_text.delete(1.0, tk.END)
        output_text.insert(tk.END, f"Translated Text in Hindi: {translated_text}\n")
    except Exception as e:
        output_text.insert(tk.END, f"Error: {e}\n")

# Function to translate and speak text in Marathi
def translate_and_speak_marathi():
    user_input = text_entry.get()
    ...
    try:
        ...
        engine.runAndWait()
        output_text.delete(1.0, tk.END)
        output_text.insert(tk.END, f"Translated Text in Marathi: {translated_text}\n")
    except Exception as e:
        output_text.insert(tk.END, f"Error: {e}\n")
...
speak_hindi_button = tk.Button(root, text="Translate and Speak in Hindi", command=translate_and_speak_hindi)
speak_hindi_button.pack(pady=10)

speak_marathi_button = tk.Button(root, text="Translate and Speak in Marathi", command=translate_and_speak_marathi)
speak_marathi_button.pack(pady=10)
